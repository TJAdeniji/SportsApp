...
//		model.addAttribute("team", team);
	...
	@RequestMapping("/football/{id}/edit")
	public String editComment(Model model, @PathVariable Long id, HttpSession session) {
		User user = this.userService.findCurrentUser(session);
		if (user == null)
			return "redirect:/login";
		
		model.addAttribute("updateComment", this.service.find(id));
		return "editComment.jsp";
	}
	
	@PostMapping("/update")
	public String update(@Valid @ModelAttribute("updateComment") Comment comment, BindingResult result, HttpSession session) {
		User user = this.userService.findCurrentUser(session);
		if (user == null)
			return "redirect:/login";
